...
//             nftDelta: nftDelta,
//             balanceDelta: 0 // Only NFT update in this call
//         });
...
//             blockNumber: startBlock + 1,
//             nftDelta: 0,
//             balanceDelta: int256(initialDeposit)
//         });
//         bytes memory sig1 = _signUserBalanceUpdates(user, update1, nonce0, DEFAULT_FOUNDRY_PRIVATE_KEY);
//         rewardsController.processUserBalanceUpdates(DEFAULT_FOUNDRY_SENDER, user, update1, sig1);
...
//         int256 nftDeltaC1 = 1;
//         int256 balanceDeltaC1 = 100 ether;

//         // Update 2: Non-whitelisted C3 (NFT +1, Deposit +50)
//         int256 nftDeltaC3 = 1;
//         int256 balanceDeltaC3 = 50 ether;
...
//             nftDelta: nftDeltaC1,
//             balanceDelta: 0 // Process NFT first
//         });
...
//             nftDelta: 0, // No NFTs
//             balanceDelta: int256(depositAmount)
//         });
...
//     // Test function specifically for processUserBalanceUpdates
//     function testProcessUserBalanceUpdates() public {
//         address user = USER_A;
//         address collection = NFT_COLLECTION_1;
//         uint256 startBlock = block.number;
//         uint256 updateBlock = startBlock + 1;
...
//             blockNumber: updateBlock,
//             nftDelta: nftDelta,
//             balanceDelta: balanceDelta // Corrected field name
//         });

//         // Sign the update
//         uint256 nonce = rewardsController.authorizedUpdaterNonce(DEFAULT_FOUNDRY_SENDER);
//         bytes memory signature = _signUserBalanceUpdates(user, updates, nonce, ownerPrivateKey);

//         // Warp to the update block
//         vm.warp(updateBlock);

//         // Expect the event
//         vm.expectEmit(true, false, false, false, address(rewardsController));
//         emit IRewardsController.UserBalanceUpdatesProcessed(user, nonce, 1);

//         // Call the function
//         rewardsController.processUserBalanceUpdates(DEFAULT_FOUNDRY_SENDER, user, updates, signature);

//         // Verify state changes
//         (uint256 finalNFTBalance, uint256 finalDepositAmount, uint256 finalUpdateBlock) =
//             rewardsController.userNFTData(user, collection);
//         assertEq(finalNFTBalance, 1, "Final NFT balance mismatch");
//         assertEq(finalDepositAmount, 100 ether, "Final deposit amount mismatch");
//         assertEq(finalUpdateBlock, updateBlock, "Final update block mismatch");

//         // Verify nonce increment
//         assertEq(rewardsController.authorizedUpdaterNonce(DEFAULT_FOUNDRY_SENDER), nonce + 1, "Nonce did not increment");
//     }

//     // Test helper function _hashBalanceUpdates
//     function test_hashBalanceUpdates() public {
//         // Use fully qualified name
...
//             blockNumber: 100,
//             nftDelta: 1,
//             balanceDelta: 10 ether // Corrected field name
//         });
//         updates[1] = IRewardsController.BalanceUpdateData({
//             collection: NFT_COLLECTION_2,
//             blockNumber: 101,
//             nftDelta: -1,
//             balanceDelta: -5 ether // Corrected field name
//         });

//         bytes32 hash = _hashBalanceUpdates(updates);

//         // Calculate expected hash manually (or via another method if complex)
//         bytes32 dataHash0 = keccak256(
//             abi.encode(
//                 BALANCE_UPDATE_DATA_TYPEHASH,
//                 updates[0].collection,
//                 updates[0].blockNumber,
//                 updates[0].nftDelta,
//                 updates[0].balanceDelta // Corrected field name
//             )
//         );
//         bytes32 dataHash1 = keccak256(
//             abi.encode(
//                 BALANCE_UPDATE_DATA_TYPEHASH,
//                 updates[1].collection,
//                 updates[1].blockNumber,
//                 updates[1].nftDelta,
//                 updates[1].balanceDelta // Corrected field name
//             )
//         );
//         bytes32 expectedHash = keccak256(abi.encodePacked(dataHash0, dataHash1));

//         assertEq(hash, expectedHash, "Hash mismatch for BalanceUpdateData array");
//     }

//     // Test helper function _signUserBalanceUpdates (implicitly tests _hashUserBalanceUpdates)
//     function test_hashUserBalanceUpdates() public {
//         address user = USER_A;
//         uint256 nonce = 123;
//         uint256 privateKey = DEFAULT_FOUNDRY_PRIVATE_KEY;
...
//             nftDelta: 1,
//             balanceDelta: 0 // Corrected field name
//         });

//         bytes memory signature = _signUserBalanceUpdates(user, updates, nonce, privateKey);

//         // Verify the signature using the contract's logic (or ECDSA.recover)
//         bytes32 updatesHash = _hashBalanceUpdates(updates);
//         bytes32 structHash = keccak256(abi.encode(USER_BALANCE_UPDATES_TYPEHASH, user, updatesHash, nonce));
//         bytes32 digest = keccak256(abi.encodePacked("\x19\x01", _buildDomainSeparator(), structHash));

//         address recoveredSigner = ECDSA.recover(digest, signature);
//         assertEq(recoveredSigner, DEFAULT_FOUNDRY_SENDER, "Recovered signer mismatch"); // Default sender has the default PK
//     }

//     // Helper function to apply delta for assertions
//     function _applyDelta(uint256 value, int256 delta) internal pure returns (uint256) {
//         if (delta >= 0) {
//             return value + uint256(delta);
//         } else {
//             uint256 absDelta = uint256(-delta);
//             require(absDelta <= value, "Underflow");
