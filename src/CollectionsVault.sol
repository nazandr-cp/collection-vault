    ...
    Pausable
{
    ...
    function setLendingManager(
        ...
        emit LendingManagerChanged(
            oldLendingManagerAddress,
            _lendingManagerAddress,
            _msgSender()
        );
    }

    function setRewardsControllerRole(
        address newRewardsController
    ) external onlyRole(ADMIN_ROLE) whenNotPaused {
    ...
    function depositForCollection(
        uint256 assets,
        address receiver,
        address collectionAddress
    ) public virtual nonReentrant whenNotPaused returns (uint256 shares) {
        shares = previewDeposit(assets);
        _deposit(msg.sender, receiver, assets, shares);
        _hookDeposit(assets);
        collectionTotalAssetsDeposited[collectionAddress] += assets;
        emit CollectionDeposit(
            collectionAddress,
            msg.sender,
            receiver,
            assets,
    ...
    function mintForCollection(
        uint256 shares,
        address receiver,
        address collectionAddress
    ) public virtual nonReentrant whenNotPaused returns (uint256 assets) {
    ...
    function withdrawForCollection(
        ...
        uint256 collectionBalance = collectionTotalAssetsDeposited[
            collectionAddress
        ];
        ...
        collectionTotalAssetsDeposited[collectionAddress] =
            collectionBalance -
            assets;
        emit CollectionWithdraw(
            collectionAddress,
            msg.sender,
            receiver,
            owner,
    ...
    function redeemForCollection(
        ...
        if (isFullRedeem) {
            ...
            if (remainingDustInLM > 0) {
                uint256 redeemedDust = lendingManager.redeemAllCTokens(
                    address(this)
                );
    ...
    function _hookWithdraw(uint256 assets) internal virtual {
        ...
        if (directBalance < assets) {
            ...
            if (neededFromLM <= availableInLM) {
                if (neededFromLM > 0) {
                    bool success = lendingManager.withdrawFromLendingProtocol(
                        neededFromLM
                    );
                    if (!success) revert LendingManagerWithdrawFailed();
                    uint256 balanceAfterLMWithdraw = assetToken.balanceOf(
                        address(this)
                    );
     ...
     */
    function _validateYieldAmount(
        address collection,
        uint256 requestedAmount
    ) internal view returns (bool isValid, uint256 maxAllowed) {
        // Get collection's reward share percentage (out of 10000)
        uint16 rewardSharePercentage = collectionRewardSharePercentage[
            collection
        ];
        ...
        uint256 collectionYieldShare = (totalYield * rewardSharePercentage) /
            10000;

        // Account for yield already transferred to this collection
        uint256 alreadyTransferred = collectionYieldTransferred[collection];
        maxAllowed = collectionYieldShare > alreadyTransferred
            ? collectionYieldShare - alreadyTransferred
            : 0;
    ...
    function transferYieldBatch(
        ...
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            (bool isValidAmount, uint256 maxAllowed) = _validateYieldAmount(
                collection,
                amount
            );
            if (!isValidAmount) {
    ...
    function _redeemYieldFromLandingManager(
        address recipient,
        uint256 amount
    ) internal {
