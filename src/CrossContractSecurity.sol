...
abstract contract CrossContractSecurity is AccessControl, ReentrancyGuard {
     ...
     */
    function setMaxTransferAmount(address target, uint256 maxAmount) 
        external 
    ...
    function validateContract(address contractAddr) 
        external 
        onlyRole(Roles.ADMIN_ROLE) 
    {
    ...
    function resetCircuitBreaker(bytes32 circuitId) 
        external 
     ...
     */
    function emergencyOverride(bytes32 circuitId) 
        external 
        onlyRole(Roles.GUARDIAN_ROLE) 
    {
        CircuitBreaker storage cb = circuitBreakers[circuitId];
