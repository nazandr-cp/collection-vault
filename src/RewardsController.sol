    ...
    PausableUpgradeable
{
    ...
    function _calculateUserWeight(address user, address collectionAddress, address forVault)
        internal
        view
        returns (uint128)
    {
        if (user == address(0) || collectionAddress == address(0) || forVault == address(0)) {
            return 0;
        }
        if (!_isCollectionWhitelisted[forVault][collectionAddress]) {
            return 0;
        }

        IRewardsController.CollectionType collectionType = _collectionType[forVault][collectionAddress];
        IRewardsController.RewardBasis rewardBasis = _collectionRewardBasis[forVault][collectionAddress];
        IRewardsController.WeightFunction memory wf = _collectionWeightFunctions[forVault][collectionAddress];
