...
library AccessControlModifiers {
    ...
    function isAdmin(
        ...
        return accessControl.hasRole(accessControl.DEFAULT_ADMIN_ROLE(), account) ||
               accessControl.hasRole(Roles.OWNER_ROLE, account) ||
               accessControl.hasRole(Roles.ADMIN_ROLE, account);
    }
     ...
     */
    function canPerformEmergencyActions(
        AccessControl accessControl,
        address account
    ) internal view returns (bool) {
        return accessControl.hasRole(Roles.GUARDIAN_ROLE, account) ||
               accessControl.hasRole(Roles.OWNER_ROLE, account) ||
    ...
    function validateRoleAssignment(
        ...
        if (role == Roles.ADMIN_ROLE || role == Roles.OWNER_ROLE || role == accessControl.DEFAULT_ADMIN_ROLE()) {
            require(
                accessControl.getRoleMemberCount(role) > 0,
                "AccessControlModifiers: Cannot remove last admin"
            );
        }
    }
     ...
     * @param account Account to check
     */
    function requireRoleWhenNotPaused(
        AccessControl accessControl,
        Pausable pausable,
        bytes32 role,
        address account
     ...
     */
    function requireEmergencyAccess(
        AccessControl accessControl,
        address account
    ) internal view {
