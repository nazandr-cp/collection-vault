...
contract EchidnaEpochManager {
    ...
    constructor() {
        ...
        // Deploy EpochManager
        epochManager = new EpochManager(EPOCH_DURATION, automatedSystem, admin, address(debtSubsidizer));
    ...
    function echidna_only_one_active_epoch() external view returns (bool) {
        ...
        try epochManager.getEpochDetails(currentEpochId) returns (
            uint256, uint256, uint256, uint256, uint256, IEpochManager.EpochStatus status
        ) {
            ...
            if (status == IEpochManager.EpochStatus.Active) {
                ...
                for (uint256 i = 1; i < currentEpochId; i++) {
                    try epochManager.getEpochDetails(i) returns (
                        uint256, uint256, uint256, uint256, uint256, IEpochManager.EpochStatus prevStatus
                    ) {
    ...
    function echidna_valid_epoch_transitions() external view returns (bool) {
        ...
        for (uint256 i = 1; i <= currentEpochId; i++) {
            try epochManager.getEpochDetails(i) returns (
                ...
                if (
                    epochStatusHistory[i] == IEpochManager.EpochStatus.Completed
    ...
    function echidna_epoch_duration_consistent() external view returns (bool) {
        ...
        for (uint256 i = 1; i <= currentEpochId; i++) {
            try epochManager.getEpochDetails(i) returns (
                uint256, uint256 startTime, uint256 endTime, uint256, uint256, IEpochManager.EpochStatus
            ) {
    ...
    function echidna_active_epoch_timestamps_reasonable() external view returns (bool) {
        ...
        try epochManager.getEpochDetails(currentEpochId) returns (
            uint256, uint256 startTime, uint256 endTime, uint256, uint256, IEpochManager.EpochStatus status
        ) {
            if (status == IEpochManager.EpochStatus.Active) {
    ...
    function echidna_yield_allocation_monotonic() external view returns (bool) {
        ...
        try epochManager.getEpochDetails(currentEpochId) returns (
            uint256, uint256, uint256, uint256 totalYieldAvailable, uint256, IEpochManager.EpochStatus
        ) {
        ...
        for (uint256 i = 1; i <= currentEpochId; i++) {
            try epochManager.getEpochDetails(i) returns (
                ...
                if (status == IEpochManager.EpochStatus.Completed) {
                    if (totalSubsidiesDistributed > totalYieldAvailable + 1e18) {
